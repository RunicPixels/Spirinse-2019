Shader "Ronald/OutlineGlow" {
    Properties {
        [HDR]_Color("Main Color", Color) = (.5,.5,.5,1)
        _MainTex("Base (RGBA)", 2D) = "white" { }
        _Ramp("Toon Ramp (RGB)", 2D) = "gray" {} 
        [HDR]_Color2("Aura Color", Color) = (0,0,1,1)
        [HDR]_ColorR("Rim Color", Color) = (0,1,1,1)
        _Outline("Outline width", Range(.002, 3.2)) = .3
        _OutlineZ("Outline Z", Range(-.06, 0)) = -.05
        _NoiseTex("Noise Texture", 2D) = "white" { }
        _Scale("Noise Scale", Range(0.0, 100)) = 0.01
